...
func main() {
	...
	http.HandleFunc("/api/pin", withMiddleware(handlePinMessage))
	http.HandleFunc("/api/unpin", withMiddleware(handleUnpinMessage))
	http.HandleFunc("/api/pinned", withMiddleware(handleGetPinned))
...
func createTables() {
	queries := []string{
		...
		`CREATE TABLE IF NOT EXISTS pinned_messages (id SERIAL PRIMARY KEY, chat_id INTEGER NOT NULL, message_id INTEGER NOT NULL, pinned_by TEXT NOT NULL, text TEXT DEFAULT '', UNIQUE(chat_id, message_id))`,
	}
...
func handleReactions(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		// Отправляем обновление реакций всем участникам чата
		go func() {
			var chatID int
			db.QueryRow("SELECT chat_id FROM messages WHERE id = $1", req.MessageID).Scan(&chatID)
			broadcast <- Message{ID: req.MessageID, ChatID: chatID, Type: "reaction_update"}
		}()
		return
	}
...
func handlePinMessage(w http.ResponseWriter, r *http.Request) {
	...
	db.Exec("INSERT INTO pinned_messages (chat_id, message_id, pinned_by, text) VALUES ($1, $2, $3, $4) ON CONFLICT (chat_id, message_id) DO NOTHING", req.ChatID, req.MessageID, currentUser, req.Text)
	json.NewEncoder(w).Encode(map[string]bool{"success": true})
}
...
func handleGetPinned(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	for rows.Next() {
		var msgID int
		var text string
		rows.Scan(&msgID, &text)
		pinned = append(pinned, map[string]interface{}{"id": msgID, "text": text})
	}
	if pinned == nil {
