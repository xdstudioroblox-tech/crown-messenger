...
func main() {
	...
	http.HandleFunc("/api/upload-voice", withMiddleware(handleUploadVoice))
	...
	http.HandleFunc("/api/reactions", withMiddleware(handleReactions))
...
func handleUploadVoice(w http.ResponseWriter, r *http.Request) {
	...
	filename := "voice_" + currentUser + "_" + strconv.FormatInt(time.Now().UnixNano(), 10) + ext
	os.WriteFile("uploads/voice/"+filename, fileBytes, 0644)
	fileURL := "/uploads/voice/" + filename

	if cloudinaryCloudName != "" {
		if cloudURL, err := uploadToCloudinary(fileBytes, "voice"); err == nil {
			fileURL = cloudURL
		}
	}
...
func getAvatarURL(a string) string {
	if a == "" { return "" }
	if strings.HasPrefix(a, "http") { return a }
	return "/uploads/" + a
}
...
func handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if !isValidUsername(req.Username) {
		json.NewEncoder(w).Encode(AuthResponse{Success: false, Error: "Юзернейм: от 6 символов, буквы, цифры, _"}); return
	}
	if len(req.Password) < 8 {
		json.NewEncoder(w).Encode(AuthResponse{Success: false, Error: "Пароль: минимум 8 символов"}); return
	}
	var existingID int
...
func handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	var users []User
	for rows.Next() {
		var u User; var email, about, avatar, phone sql.NullString
		rows.Scan(&u.ID, &u.Username, &u.Nickname, &email, &about, &avatar, &phone)
		u.Email = email.String; u.About = about.String; u.Avatar = getAvatarURL(avatar.String); u.Phone = phone.String
	...
	for grows.Next() {
		var g GroupInfo; grows.Scan(&g.ID, &g.Name, &g.Avatar, &g.CreatedBy)
		g.Avatar = getAvatarURL(g.Avatar); g.IsGroup = true; groups = append(groups, g)
	}
...
func handleUploadAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if cloudinaryCloudName != "" { if cloudURL, err := uploadToCloudinary(fileBytes, "avatars"); err == nil { fileURL = cloudURL } }
...
func handleLastSeen(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" { json.NewEncoder(w).Encode(map[string]interface{}{"online": false, "last_seen": ""}); return }
	mu.Lock(); _, online := onlineUsers[username]; mu.Unlock()
	var lastSeen sql.NullString
	db.QueryRow("SELECT COALESCE(last_seen::text, '') FROM users WHERE username = $1", username).Scan(&lastSeen)
	json.NewEncoder(w).Encode(map[string]interface{}{"online": online, "last_seen": lastSeen.String})
}
...
func handleCreateGroup(w http.ResponseWriter, r *http.Request) {
	...
	currentUser := getUserFromRequest(r)
	if currentUser == "" { http.Error(w, "Unauthorized", http.StatusUnauthorized); return }
	var req struct { Name string `json:"name"`; Public bool `json:"public"` }
	json.NewDecoder(r.Body).Decode(&req)
...
func handleUpdateGroup(w http.ResponseWriter, r *http.Request) {
	...
	currentUser := getUserFromRequest(r)
	if currentUser == "" { http.Error(w, "Unauthorized", http.StatusUnauthorized); return }
	var req struct { ID int `json:"id"`; Name string `json:"name,omitempty"`; Description string `json:"description,omitempty"`; Public bool `json:"public,omitempty"` }
	json.NewDecoder(r.Body).Decode(&req)
...
func handleGroupLeave(w http.ResponseWriter, r *http.Request) {
	...
	currentUser := getUserFromRequest(r)
	if currentUser == "" { http.Error(w, "Unauthorized", http.StatusUnauthorized); return }
	var req struct { GroupID int `json:"group_id"` }
	json.NewDecoder(r.Body).Decode(&req)
...
func handleStickerUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { log.Printf("❌ Ошибка получения файла стикера: %v", err); json.NewEncoder(w).Encode(map[string]string{"error": "Файл не найден: " + err.Error()}); return }
	defer file.Close()
	fileBytes, _ := io.ReadAll(file)
	packIDStr := r.FormValue("pack_id")
	packID, err := strconv.Atoi(packIDStr)
	if err != nil { json.NewEncoder(w).Encode(map[string]string{"error": "Неверный pack_id"}); return }
	...
	if cloudinaryCloudName != "" {
		...
		if err != nil { log.Printf("❌ Ошибка Cloudinary: %v", err); json.NewEncoder(w).Encode(map[string]string{"error": "Ошибка загрузки в Cloudinary: " + err.Error()}); return }
		fileURL = cloudURL; log.Printf("✅ Стикер загружен в Cloudinary: %s", fileURL)
	} else {
		ext := filepath.Ext(header.Filename); filename := "sticker_" + currentUser + "_" + strconv.FormatInt(time.Now().UnixNano(), 10) + ext
		os.WriteFile("uploads/stickers/"+filename, fileBytes, 0644); fileURL = "/uploads/stickers/" + filename
	}
...
func handleStickerList(w http.ResponseWriter, r *http.Request) {
	...
	if packID == "" { json.NewEncoder(w).Encode([]Sticker{}); return }
...
func handleGifUpload(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { json.NewEncoder(w).Encode(map[string]string{"error": "Файл не найден"}); return }
	defer file.Close()
	fileBytes, _ := io.ReadAll(file)
	ext := filepath.Ext(header.Filename); filename := "gif_" + currentUser + "_" + strconv.FormatInt(time.Now().UnixNano(), 10) + ext
	os.WriteFile("uploads/gifs/"+filename, fileBytes, 0644); fileURL := "/uploads/gifs/" + filename
	if cloudinaryCloudName != "" { if cloudURL, err := uploadToCloudinary(fileBytes, "gifs"); err == nil { fileURL = cloudURL } }
...
func handleStickerPacks(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		req.Name = escapeHTML(strings.TrimSpace(req.Name))
		if req.Name == "" { json.NewEncoder(w).Encode(map[string]string{"error": "Название обязательно"}); return }
		var packID int; db.QueryRow("INSERT INTO sticker_packs (name, owner) VALUES ($1, $2) RETURNING id", req.Name, currentUser).Scan(&packID)
...
func handleConnections(w http.ResponseWriter, r *http.Request) {
	...
	defer ws.Close()
	mu.Lock()
	clients[ws] = username; onlineUsers[username] = true
...
func sendChatNotificationIfNew(username string, chatID int, nickname string) {
	if chatID == 1 { return }
	var u1, u2 string; db.QueryRow("SELECT user1, user2 FROM chats WHERE id = $1", chatID).Scan(&u1, &u2)
	peer := u1; if peer == username { peer = u2 }
	var count int; db.QueryRow("SELECT COUNT(*) FROM messages WHERE chat_id = $1", chatID).Scan(&count)
