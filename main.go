...
func handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		json.NewEncoder(w).Encode(AuthResponse{Success: false, Error: "Ошибка сервера"})
		return
	}
	var userID int
	db.QueryRow("INSERT INTO users (username, nickname, password, email, phone) VALUES ($1, $2, $3, $4, $5) RETURNING id",
		req.Username, req.Nickname, string(hashedPassword), req.Email, req.Phone).Scan(&userID)
...
func handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow("SELECT id, username, nickname, password, email, about, avatar, phone FROM users WHERE username = $1",
		req.Username).Scan(&user.ID, &user.Username, &user.Nickname, &hashedPassword, &email, &about, &avatar, &phone)
	if err != nil {
	...
	if bcrypt.CompareHashAndPassword([]byte(hashedPassword.String), []byte(req.Password)) == nil {
		user.Email = email.String; user.About = about.String
		user.Avatar = getAvatarURL(avatar.String); user.Phone = phone.String
...
func handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	for grows.Next() {
		...
		g.Avatar = getAvatarURL(g.Avatar); g.IsGroup = true
...
func handleMessagesAPI(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		...
		if chatID == "" { chatID = "1" }
		rows, _ := db.Query("SELECT id, username, nickname, text, time, avatar, COALESCE(read,false), COALESCE(edited,false), COALESCE(file_url,''), COALESCE(file_type,''), COALESCE(reply_to,0), COALESCE(reply_text,''), COALESCE(reply_nick,'') FROM messages WHERE chat_id = $1 ORDER BY id ASC LIMIT 100", chatID)
		defer rows.Close()
...
func handleBlockUser(w http.ResponseWriter, r *http.Request) {
	...
	if req.Block {
		...
		mu.Lock()
		if blockedUsers[currentUser] == nil { blockedUsers[currentUser] = make(map[string]bool) }
		...
		mu.Lock()
		if blockedUsers[currentUser] != nil { delete(blockedUsers[currentUser], req.Username) }
		mu.Unlock()
	}
	json.NewEncoder(w).Encode(map[string]bool{"success": true})
}
...
func handleChatList(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		rows.Scan(&id, &u1, &u2)
		peer := u1; if peer == currentUser { peer = u2 }
...
func handleCreateGroup(w http.ResponseWriter, r *http.Request) {
	...
	var groupID int
	err := db.QueryRow("INSERT INTO groups_chat (name, created_by, created_at, invite_code, public) VALUES ($1, $2, $3, $4, $5) RETURNING id",
		req.Name, currentUser, time.Now().Format("2006-01-02 15:04"), inviteCode, req.Public).Scan(&groupID)
...
func handleConnections(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		db.Exec("INSERT INTO messages (username, nickname, text, time, chat_id, avatar, read, edited, file_url, file_type, reply_to, reply_text, reply_nick) VALUES ($1, $2, $3, $4, $5, $6, false, false, $7, $8, $9, $10, $11)",
			msg.Username, msg.Nickname, msg.Text, msg.Time, msg.ChatID, avatar, msg.FileURL, msg.FileType, msg.ReplyTo, msg.ReplyText, msg.ReplyNick)
		broadcast <- msg
	}
}
